...
def patientSearchCriteria():
    ...
    op = request.form.get('option', False)
    if op == 'fname':
    ...
    lower(name) LIKE '%" + searchCriteria.lower() + "%' order by userId desc limit 10")
       ...
       searchResults = query_db("SELECT userId, gender FROM users WHERE userType = 'patient' AND \
    lower(gender) LIKE '%" + searchCriteria.lower() + "%' order by userId desc limit 10")
    elif op == 'dob':
        searchResults = query_db("SELECT userId, dob FROM users WHERE userType = 'patient' AND \
    lower(dob) LIKE '%" + searchCriteria.lower() + "%' order by userId desc limit 10")
    else:
        searchResults = query_db("SELECT userId, name FROM users WHERE userType = 'patient' AND \
    lower(name) LIKE '%" + searchCriteria.lower() + "%' order by userId desc limit 10")
...
def formProcess():
    if request.method == 'POST':
        try:
            ...
            insertQuery("INSERT INTO users (gender, dob) \
